...
// ThermalDLASystem is a contained system for modelling Diffusion Limited Aggregation
type ThermalDLASystem struct {
...
// NewThermalDLASystem initialises a new DLA system
func NewThermalDLASystem(gridSize int, addRatio float64, killRatio float64, endNumberOfParticles int, seed int64, verbose bool) *ThermalDLASystem {
	dla := new(ThermalDLASystem)
	dla.verbose = verbose
	dla.gridSize = gridSize
	dla.addRatio = addRatio
	dla.killRatio = killRatio
	dla.endNumberOfParticles = endNumberOfParticles
	dla.stickingProbability = 1.0
...
func (dla *ThermalDLASystem) DisplayGrid() {
	fmt.Printf("\n")
...
func (dla *ThermalDLASystem) Reset() {
	dla.isRunning = false
	dla.ClearParticles()
...
func (dla *ThermalDLASystem) ClearParticles() {
...
func (dla *ThermalDLASystem) Update() {
...
func (dla *ThermalDLASystem) DetermineIfShouldStickInsteadOfRecoil() bool {
	dla.stickingProbability = 0.5 + 0.4*math.Sin(2*math.Pi*float64(dla.numberOfParticles)/float64(dla.endNumberOfParticles+1))
...
func (dla *ThermalDLASystem) MoveLastParticle() {
	if dla.verbose {
...
func (dla *ThermalDLASystem) UpdateClusterRadius(lastParticlePosition [2]int) {
...
func (dla *ThermalDLASystem) CheckIfShouldStop() bool {
...
func (dla *ThermalDLASystem) PauseSimulation() {
	dla.isRunning = false
}
...
func (dla *ThermalDLASystem) ShouldParticleStick(particle *Particle) bool {
...
func (dla *ThermalDLASystem) AddParticleOnAddCircle() {
	if dla.verbose {
...
func (dla *ThermalDLASystem) AddParticle(position [2]int) {
...
func (dla *ThermalDLASystem) SetGrid(position [2]int, isOccupied bool) {
...
func (dla *ThermalDLASystem) ReadGrid(position [2]int) bool {
	halfGrid := dla.gridSize / 2
	return dla.grid[position[0]+halfGrid][position[1]+halfGrid]
}

// PersistGridToFile persists the state of the grid to disc
func (dla *ThermalDLASystem) PersistGridToFile(filename string) {
